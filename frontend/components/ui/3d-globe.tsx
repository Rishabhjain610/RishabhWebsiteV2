...
export interface GlobeMarker {
  ...
  label?: string;
  size?: number;
  color?: string; // Custom color for the marker beacon
}
...
function Marker({
  ...
  const beaconColor = marker.color || (hovered ? "#60A5FA" : "#4A90E2");
  ...
  useFrame(({ clock }) => {
    ...
    if (ringRef.current) {
      ...
      const s = 1 + (Math.sin(t) + 1) * 0.5; // Scale from 1 to 2
      ringRef.current.scale.set(s, s, s);
      if (ringRef.current.material instanceof THREE.MeshBasicMaterial) {
        ringRef.current.material.opacity = 0.5 * (1 - (s - 1)); // Fade out as it grows
      }
    }
  });
  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      ...
      {/* Pin point at the surface with glow */}
      ...
      <mesh ref={ringRef} position={surfacePosition} rotation-x={Math.PI / 2}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            ...
            style={{
              width: "18px", // Larger for better logo visibility
              height: "18px",
            }}
...
function RotatingGlobe({
  ...
  const geometry = useMemo(() => {
    return new THREE.SphereGeometry(config.radius, 128, 64); // Smoother mesh
  }, [config.radius]);
    ...
    <group ref={groupRef}>
      ...
      <mesh geometry={geometry}>
        <meshStandardMaterial
          ...
          roughness={0.4} // Slicker surface
          metalness={0.1} // Premium feel
        />
      </mesh>
...
const defaultConfig: Required<Globe3DConfig> = {
  ...
  ambientIntensity: 0.8, // Increased for clarity
  pointLightIntensity: 2.5, // Brighter highlight
