...
const projects: Project[] = [
  {
    ...
    features: [
      "Architected distributed 4-service microservices with API Gateway orchestrating Auth, Billing, Chat, Agent services; implemented REST APIs and Redis sync enabling fault isolation and elastic scaling across distributed systems",
      "Built RAG pipeline using Nomic embeddings + Qdrant vector search for document retrieval with memory; orchestrated 8 LangGraph agents for Chat, Coding, Web Search, RAG, Image Analysis, Image/PDF/PPT Generation",
      "Implemented BullMQ job queue for async processing and Razorpay billing with sliding-window rate limiting",
  ...
  },
  {
    ...
    features: [
      "Built full-stack social platform supporting posts, reels, real-time Socket.io chat with AI chat summarizer, and Clerk authentication; sustained 150+ concurrent users at 447 req/sec",
      "Optimized API response time by 99% (2.3s to 21 ms) using database indexing, Redis caching, pagination, and IP-based rate limiting; containerized via Docker with AWS S3",
      "Built multi-agent orchestration: Planner Agent classifies intent and injects last 3 conversation messages as context, routing to specialized agents; Analytics agent (6 tools: profile, posts, followers, engagement, growth, charts), Content agent (6 tools: Tavily search, Unsplash images, Pollinations art, captions, hashtags, publish with Human-in-the-Loop)",
  ...
  },
  {
    ...
    features: [
      "Developed a 9-module AI-powered digital campus platform featuring Smart Chat Room, AI PDF Assistant, Academic Calendar, AI Marks Analyzer, AI Search Agent, E-Library, E-Canteen, and Railway Concession Generator",
      "Implemented dual authentication (Email/Password + Google OAuth) with RBAC, real-time Socket.io messaging, OCR-based document understanding, context-aware note Q&A, conversation summarization, and Excel-based analytics",
  ...
  },
  {
    ...
    features: [
      "Built multi-modal incident reporting with a 4-phase forensics pipeline (EXIF, Tesseract OCR, Gemma 3 LLM) backed by MongoDB 2dsphere indexing to drop query complexity from O(n) to O(log n)",
      "Architected a React-Leaflet dashboard utilizing input-aware trust-scoring formulas to seamlessly map threat data",
